---
title: "Policy as Code - A Simple Introduction"
subtitle: "From PDF to Pipeline: Rules That Run Themselves"
author: "Patrick Lefler"
date: "April 30, 2026"
abstract: "Most compliance programs still run on documents. A rule gets written in prose, approved through committee, filed somewhere, and consulted — if it is consulted at all — by a human who must decide what it means. That model has a well-documented failure mode: inconsistent interpretation, undetected drift, and a chronic gap between what the policy says and what the systems actually do.</br> </br>
Policy as Code replaces the document with executable logic. The rule becomes a function: it accepts inputs, evaluates conditions, and returns a deterministic verdict that a machine can enforce, a developer can test, and an auditor can trace back through version history. The approach is not new — firewall rules and access control lists have always been policies expressed as code — but the pattern is now being applied more broadly to data governance, infrastructure configuration, and business approval workflows.</br> </br>
This document introduces the concept through a deliberately simple example: a C# password validation function built with regular expressions. The simplicity is the point. The same structure that governs twelve characters scales to the problems that keep boards up at night."
format:
  html:
    code-fold: true
    code-copy: false
...
Step 1: Is the length ≥ 12 characters? </br> `string basicRegex = "^.{8,}$";`

Step 2: Is the length ≤ 25 characters? </br> `string basicRegex = "^.{,25}$";`

Step 3: Does it contain at least one uppercase letter? </br> `string basicRegex = (?=.*[A-Z])`

Step 4: Does it contain at least one lowercase letter? </br> `string basicRegex = (?=.*[a-z])`

Step 5: Does it contain a nummeric digit? </br> `string basicRegex = (?=.*\d)`

Step 6: Does it contain a special character? </br> `string basicRegex = (?=.*[@$!%*?&])`

Putting it all together becomes a single line of basicRegex: </br> `string advancedRegex = "^{12,25}(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])$";`
...
*Rendered with [Quarto](https://quarto.org/) + [Mermaid](https://mermaid.js.org/) | Packages: `dplyr` `kableExtra` `knitr` `scales` `sessioninfo` `tidyverse`*
